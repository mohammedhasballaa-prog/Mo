...
class MainActivity : Activity() {
    ...
    private fun show() {
        ...
        head.text = "📚 مذكرتي\nكل اللي اتعلمته وعملته في مكان واحد"
        head.textSize = 22f
        head.setTextColor(Color.WHITE)
        head.setPadding(16, 16, 16, 16)
        head.setBackgroundColor(Color.rgb(25, 25, 25))
        root.addView(head)
        ...
        val add = Button(this)
        add.text = if (section == "work") "➕ إضافة وصفة" else "➕ إضافة ملاحظة"
        add.setOnClickListener { editor(-1) }
        root.addView(add)
        ...
        items.forEachIndexed { idx, o ->
            if (o.toString().contains(q.text.toString(), true)) {
                ...
                val t = TextView(this)
                t.text = o.optString("title")
                t.textSize = 19f
                card.addView(t)

                val body = TextView(this)
                body.text = if (section == "work") {
                ...
                val e = Button(this)
                e.text = "✏️ تعديل"
                e.setOnClickListener { editor(idx) }
    ...
    private fun editor(index: Int) {
        ...
        title.hint = "العنوان"
        title.setText(old.optString("title"))
        box.addView(title)

        val ing = EditText(this)
        ing.hint = "المكونات (للوصفات)"
        ing.setText(old.optString("ingredients"))
        if (section != "work") ing.visibility = View.GONE
        box.addView(ing)

        val body = EditText(this)
        body.hint = if (section == "work") "طريقة التحضير" else "ماذا استفدت؟ / الملاحظات"
        body.setMinLines(6)
        body.setText(old.optString("body"))
        box.addView(body)

        AlertDialog.Builder(this)
            .setTitle(if (index < 0) "إضافة" else "تعديل")
            .setView(box)
            .setPositiveButton("حفظ") { _, _ ->
                val o = JSONObject()
                o.put("title", title.text.toString())
                o.put("ingredients", ing.text.toString())
                o.put("body", body.text.toString())
